...
import com.huizhi.aianswering.mapper.ScoringResultMapper;
import com.huizhi.aianswering.model.dto.scoringresult.ScoringResultQueryRequest;
...
import com.huizhi.aianswering.model.vo.ScoringResultVO;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "结果的应用类别非法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 128, ErrorCode.PARAMS_ERROR, "结果名称长度不能大于128");
        }
        if (appId != null) {
            App byId = appService.getById(appId);
            ThrowUtils.throwIf(byId == null, ErrorCode.PARAMS_ERROR, "结果的应用类别非法");
        }
