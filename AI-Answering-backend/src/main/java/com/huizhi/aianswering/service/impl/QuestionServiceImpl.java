...
import com.huizhi.aianswering.mapper.QuestionMapper;
import com.huizhi.aianswering.model.dto.question.QuestionQueryRequest;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用类别非法");
        }
        // 修改数据时，有参数则校验
        // 无论创建还是修改都要先判断传过来的appId是否为空
        if (appId != null) {
            // 查询数据库中appId是否存在
            App byId = appService.getById(appId);
            ThrowUtils.throwIf(byId == null, ErrorCode.PARAMS_ERROR, "此应用不存在");
        }
    }
