...
import com.huizhi.aianswering.manager.AiManager;
import com.huizhi.aianswering.model.dto.question.QuestionAnswerDTO;
...
public class AiTestScoringStrategy implements ScoringStrategy {
    ...
    public UserAnswer doScoring(List<String> choices, App app) throws Exception {
        ...
        // 定义锁
        RLock answerLock = redissonClient.getLock(AI_ANSWER_LOCK + cacheKey);
        try {
            boolean b = answerLock.tryLock(3, 15, TimeUnit.SECONDS);
            // 如果没有拿到锁直接返回空或设置报错
            if (!b) {
                return null;
            }
            ...
            if (answerLock != null && answerLock.isLocked()) {
                if (answerLock.isHeldByCurrentThread()) {
                    answerLock.unlock();
                }
            }
        }
